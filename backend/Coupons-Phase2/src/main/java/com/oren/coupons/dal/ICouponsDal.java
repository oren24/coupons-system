...
public interface ICouponsDal extends CrudRepository<CouponEntity, Integer> {
    ...
    @Query("select c from CouponEntity c")
    public List<CouponEntity> getAllCouponsEntitys();

    @Query("select c from CouponEntity c where c.company.id = :id")
    public List<Coupon> getAllCouponsByCompanyId(@Param("id") int id);

    @Query("select c from CouponEntity c where c.endDate <= :endDate")
    public List<Coupon> getAllCouponsByEndDate(@Param("endDate") Date endDate);
    @Query("select c from CouponEntity c where c.startDate >= :startDate")
    public List<Coupon> getAllCouponsByStartDate(@Param("startDate") java.sql.Date startDate);

    @Query("select c from CouponEntity c where c.price <= :maxPrice")
    public List<Coupon> getAllCouponsUpToPrice(@Param("maxPrice") float maxPrice);

    @Query("select c from CouponEntity c where c.amount >= :amount")
    public List<Coupon> getAllCouponsUpToAmount(@Param("amount") int amount);

    @Query("select c from CouponEntity c where c.category.id = :categoryId")
    public List<Coupon> getAllCouponsByCategoryId(@Param("category") int categoryId);

    @Transactional
    @Modifying
    @Query("update CouponEntity c set c.name = null, c.description = :description where c.id = :id")
    public int updateNameAndDescriptionById(@Param("description") String description, @Param("id") int id);
    @Transactional
    @Modifying
    @Query("update CouponEntity c set c.amount = :amount where c.id = :id")
    public void updateAmountById(@Param("amount") int amount, @Param("id") int id);
